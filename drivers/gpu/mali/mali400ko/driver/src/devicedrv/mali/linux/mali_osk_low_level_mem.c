...
void mali_check_clock(void)
{
	unsigned int mgt = readl_relaxed(_PRCMU_BASE + PRCM_SGACLK_MGT);
...
{
	writel_relaxed(cpu_to_le32(val),((u8*)addr) + offset);
}

u32 inline _mali_osk_mem_ioread32( volatile mali_io_address addr, u32 offset )
{
	return readl(((u8*)addr) + offset);
}

void inline _mali_osk_mem_iowrite32( volatile mali_io_address addr, u32 offset, u32 val )
{
	writel(val, ((u8*)addr) + offset);
}
